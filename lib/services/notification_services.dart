...
class NotifyHelper {
  ...
  tz.TZDateTime _nextInstanceOfTenAM(
    ...
    var fomattedDate = DateFormat().add_yMd().parse(date);
    final tz.TZDateTime fd = tz.TZDateTime.from(fomattedDate, tz.local);
    ...
    if (scheduledDate.isBefore(now)) {
      if (repeat == 'Daily') {
        scheduledDate = tz.TZDateTime(tz.local, now.year, now.month,
            (fomattedDate.day) + 1, hour, minutes);
      }
      if (repeat == 'Weekly') {
        scheduledDate = tz.TZDateTime(tz.local, now.year, now.month,
            (fomattedDate.day) + 7, hour, minutes);
      }
      if (repeat == 'Monthly') {
        scheduledDate = tz.TZDateTime(tz.local, now.year, now.month,
            (fomattedDate.month) + 1, hour, minutes);
      }
      scheduledDate = afterRemind(remind, scheduledDate);
    }
